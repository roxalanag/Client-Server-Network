...
///
/// The user name (@u) and user password (@p) must conform to LEN_UNAME and
/// LEN_PASSWORD.  The map/reduce (@m) name must be no more than LEN_FNAME
/// characters.
///
/// @rblock   padR(enc(pubkey, "MAP__RED".aeskey.length(@ablock)))
/// @ablock   enc(aeskey, len(@u).len(@p).len(@m).null(8).@u.@p.@m)
...
const std::string REQ_KMR = "MAP__RED";

/// Allow user @u (with password @p) to register a .so (@s) containing map() and
/// reduce() functions, using the name @m.
///
/// The user name (@u) and user password (@p) must conform to LEN_UNAME and
/// LEN_PASSWORD.  @m must be no more than LEN_FNAME characters.  The user (@u)
/// must be the systen administrator.  The .so file (@s) must be no more than
/// LEN_SO bytes.
///
/// @rblock   padR(enc(pubkey, "KVREGMR_".aeskey.length(@ablock)))
/// @ablock   enc(aeskey, len(@u).len(@p).len(@m).len(@s).@u.@p.@m.@s)
/// @response enc(aeskey, "OK").<EOF>       -- Success
///           enc(aeskey, error_code).<EOF> -- Error (see @errors)
///           ERR_CRYPTO.<EOF>              -- Error (see @errors)
/// @errors   ERR_LOGIN       -- @u is not a valid user
...
///           ERR_QUOTA_UP    -- Client exceeded upload bandwidth quota
///           ERR_SERVER      -- Internal Server Error
const std::string REQ_KVF = "REG_MR__";
